/*
 * The namespace Roadbed.Net.Services was removed on purpose and replaced with Roadbed.Net so that no additional using statements are required.
 */
                ...
                using (HttpRequestMessage requestMessage = CreateHttpRequestMessage(request))
                {
                    ...
                    else
                    {
                        response = NetHttpResponse<T>.Failure(
                                (int)message.StatusCode,
                                message.ReasonPhrase,
                                "Http status code does not indicate a successful API call.");
                    }
                }
            }
